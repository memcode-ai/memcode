...
import (
	...
	"github.com/memcode-ai/memcode/internal/forks/vaxis/ansi"
	"github.com/memcode-ai/memcode/internal/forks/vaxis/log"
)
...
func (vx *Vaxis) renderPrimary() {
	...
	if primary.rendered {
		moveRows := regionRows
		if primary.resized {
			// memcode fork fix: on a region-height change move up by the OLD rendered height
			// (visualRows), not the new one. Upstream moved up by the larger NEW height on a
			// GROW, overshooting above the old region into scrollback and erasing it with
			// \x1B[J. Using the old height positions at the old region top; the paint loop's
			// \r\n then scrolls the terminal to make room, preserving scrollback.
...
func (vx *Vaxis) disableModes() {
	...
	// UNCONDITIONAL, mirroring the blind decset in sendQueries: a terminal can enable
	// mode 2048 without sending the immediate report that sets caps.inBandResize, and a
	// caps-guarded reset then leaves the mode ON after exit — every later window resize
	// sprays "CSI 48;rows;cols;h;w t" reports into the user's shell as literal garbage.
...
func (vx *Vaxis) Suspend() error {
	...
	// Swallow the terminal's replies to our teardown queries (the DA1 wake above, plus any late
	// geometry reports) while we still own the tty in raw mode and the parser is stopped — so they
	// don't leak into the user's shell as literal escape text after Reset restores cooked mode.
	if d, ok := vx.tty.(interface{ Drain() }); ok {
		d.Drain()
	}
	_ = vx.tty.Reset()
	return nil
