...
const (
	...
	UserPolicy       = "policy"            // read/set the user's behavior policy (which model does what, plan review/advisor mode, ...) — only on explicit instruction
...
type (
	...
	// PolicyInput drives the policy tool: read or change the user's behavior
	// settings. It records a choice the user stated; it never makes one.
	PolicyInput struct {
...
func Defs() []wire.ToolDef {
	return []wire.ToolDef{
		...
		{
			Name:        UserPolicy,
			Description: "Read or change how memcode behaves. `show` (default) lists the user's settings and where each came from, and names every target. `set` records a choice they stated (\"always use haiku for explore agents\", \"always review plans with grok\", \"never review plans\"); `unset` resets one. `scope`: operation (this plan) | session | workspace (default) | user. Call ONLY when they ask — NEVER on your own judgement that a task is hard or expensive. You record their choices, you do not make them.",
			InputSchema: obj(map[string]any{
				"action": enum("show (default) | set | unset", []string{"show", "set", "unset"}),
				"target": str("the decision point, e.g. agent.explore or plan.review"),
				"fields": obj(map[string]any{}),
				"scope":  enum("operation | session | workspace (default) | user", []string{"operation", "session", "workspace", "user"}),
			}),
		},
		{
